...
const ACCEPTANCE_CRITERIA: string[] = [
  ...
  'No indication recorded during Steps 2–11 exceeds the rejection criteria of the applicable standard without a documented Final Evaluation disposition.',
...
interface DrillpipeInspectionDetailsProps {
  serviceShortCode?: string;
  equipmentList?: string[];
}

export const DrillpipeInspectionDetails: React.FC<DrillpipeInspectionDetailsProps> = ({ 
  serviceShortCode = 'DS-1 CAT4',
  equipmentList = []
}) => {
    ...
    <div className="bg-[#0D244E]/90 p-4 sm:p-5 rounded-2xl border border-[#2353A1] text-xs space-y-4 shadow-xl">
      ...
      <div className="pt-1 flex items-center justify-between">
        ...
        <span className="text-[11px] font-mono text-blue-300 hidden sm:inline">
          14-Step Standardized Procedure • {serviceShortCode}
        </span>
      </div>
      ...
      <AnimatePresence initial={false}>
        {isExpanded && (
          <motion.div
            key="drillpipe-expanded-content"
          ...
          >
            ...
            <div className="space-y-2">
              <h4 className="text-base sm:text-lg font-black text-white tracking-tight flex items-center space-x-2">
                ...
                <span>Drill Pipe Inspection</span>
              </h4>
              <p className="text-blue-100 text-xs sm:text-sm leading-relaxed">
                This service line covers the full inspection of used drill pipe, from receipt and cleaning through eleven sequential inspection, treatment, and marking activities. The objective is to certify that drill pipe tube bodies, upsets, slip areas, and threaded connections remain fit for continued service in accordance with the referenced standards, and to identify and classify any items that do not meet the applicable acceptance criteria.
              </p>
            </div>
            ...
            <div className="bg-[#133870] border border-amber-400/50 rounded-xl p-3.5 flex items-start space-x-3 shadow-inner">
              ...
              <div className="text-xs space-y-0.5">
                ...
                <p className="text-blue-100 font-mono text-[11px] leading-relaxed">
                  API RP 7G-2; DS-1 Vol. 3; API Spec 5DP (dimensional baseline)
                </p>
              </div>
            </div>

            {/* 14-Step Inspection Procedure */}
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <h5 className="text-xs font-bold text-amber-300 uppercase tracking-wider flex items-center space-x-1.5">
                  <Layers className="w-3.5 h-3.5" />
                  <span>Sequential 14-Step Inspection Procedure</span>
                </h5>
              ...
              <div className="hidden md:block overflow-x-auto rounded-xl border border-[#2353A1] shadow-md">
                <table className="w-full text-left border-collapse text-xs">
                  ...
                  <tbody className="divide-y divide-[#2353A1]/60">
                    {DRILLPIPE_INSPECTION_STEPS.map((item) => (
                      <tr 
                        key={item.step} 
