...
const TUBING_INSPECTION_STEPS: InspectionStep[] = [
  {
    step: 1,
  ...
  },
  {
    ...
    description: 'Full-length visual inspection of the inside and outside surfaces of used tubes to detect corrosion, mechanical damage, and surface imperfections.',
    responsibleParty: 'NDT Inspector (Level II)',
    standardRef: 'API RP 7G-2'
  },
  {
    step: 3,
    activity: 'Visual Thread Inspection',
    description: 'Visual inspection of connections to detect pitting, mash, dent, and galling — defects that could compromise the pressure-sealing integrity of the tubing string.',
    responsibleParty: 'NDT Inspector (Level II)',
    standardRef: 'API RP 7G-2'
  },
  {
    ...
    description: 'A drift mandrel of standard dimensions is passed through the full internal length of the tube to confirm the minimum internal diameter is maintained along its entire length, verifying downhole tool and completion equipment can pass freely.',
  ...
  },
  {
    ...
    description: 'Full-length scanning (excluding external upsets) using a longitudinal-field, transverse-flaw buggy-type unit to detect body defects.',
  ...
  },
  {
    ...
    description: 'Wall thickness measured around one circumference of the tube using an ultrasonic thickness gauge, to detect wall loss from internal/external corrosion or erosion.',
  ...
  },
  {
    ...
    description: 'Full-length external coating applied.',
...
const TUBING_ACCEPTANCE_CRITERIA: string[] = [
  "All ten steps completed and recorded against the item's unique identification reference.",
  'Drift test passed along the full internal length without obstruction.',
  'Wall-thickness readings meet or exceed the minimum threshold of the applicable standard.',
  'Connections free of pitting, mash, dent, or galling exceeding acceptance limits.'
];

interface TubingInspectionDetailsProps {
  serviceShortCode?: string;
  equipmentList?: string[];
}

export const TubingInspectionDetails: React.FC<TubingInspectionDetailsProps> = ({ 
  serviceShortCode = 'API RP 74-2',
  equipmentList = ['High-Sensitivity Tubing EMI Unit', 'API Drift Mandrels', 'Thread Gauges', 'Ultrasonic Gauges']
}) => {
  const [isExpanded, setIsExpanded] = useState<boolean>(false);

  return (
    <div className="bg-[#0D244E]/90 p-4 sm:p-5 rounded-2xl border border-[#2353A1] text-xs space-y-4 shadow-xl">
      {/* Visual Tubing Images Showcase */}
      <div className="space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          <div className="relative aspect-[16/10] w-full rounded-xl overflow-hidden border border-[#2353A1] shadow-md group">
            <img 
              src="https://www.wittyservices.com/img/OCTG.png" 
              alt="Production Tubing NDT and Verification" 
          ...
          <div className="relative aspect-[16/10] w-full rounded-xl overflow-hidden border border-[#2353A1] shadow-md group">
            <img 
              src="https://www.central-steels.com/attached/image/20211105/20211105101532573257.jpg" 
              alt="Tubing Body and Thread Pressure Integrity Testing" 
              className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
              referrerPolicy="no-referrer"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[#0D244E]/80 via-transparent to-transparent pointer-events-none" />
      ...
      <div className="pt-2 flex items-center justify-between">
        ...
        <span className="text-[11px] font-mono text-blue-300 hidden sm:inline">
          10-Step Standardized Procedure • {serviceShortCode}
        </span>
      </div>
          ...
          >
            ...
            <div className="space-y-2">
              <h4 className="text-base sm:text-lg font-black text-white tracking-tight flex items-center space-x-2">
                ...
                <span>Tubing Inspection</span>
              </h4>
              <p className="text-blue-100 text-xs sm:text-sm leading-relaxed">
                Production tubing is inspected to confirm it remains fit to convey well fluids under pressure without leakage. This service line combines visual, dimensional, drift, electromagnetic, and ultrasonic methods to validate both the tube body and its threaded connections.
              </p>
            </div>
            ...
            <div className="bg-[#133870] border border-amber-400/50 rounded-xl p-3.5 flex items-start space-x-3 shadow-inner">
              ...
              <div className="text-xs space-y-0.5">
                ...
                <p className="text-blue-100 font-mono text-[11px] leading-relaxed">
                  API RP 7G-2 (methodology reference); [Client Confirmation Required — tubing-specific standard, e.g., API 5CT-aligned criteria]
                </p>
              </div>
            </div>

            {/* 10-Step Inspection Procedure */}
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <h5 className="text-xs font-bold text-amber-300 uppercase tracking-wider flex items-center space-x-1.5">
                  <Layers className="w-3.5 h-3.5" />
                  <span>Sequential 10-Step Tubing Inspection Procedure</span>
                </h5>
              ...
              <div className="hidden md:block overflow-x-auto rounded-xl border border-[#2353A1] shadow-md">
                <table className="w-full text-left border-collapse text-xs">
                  ...
                  <tbody className="divide-y divide-[#2353A1]/60">
                    {TUBING_INSPECTION_STEPS.map((item) => (
                      <tr 
                        key={item.step} 
            ...
            <div className="bg-[#0A1F42] border border-[#306AC1] rounded-xl p-4 sm:p-5 space-y-3 shadow-md">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                {TUBING_ACCEPTANCE_CRITERIA.map((criterion, cIdx) => (
                  <div 
                    key={cIdx} 
